...
import { detectIncidentsFromVideo, detectFrameFromWebcam } from '../api/incidents'
...
function LocalWebcamFeed() {
  ...
  const processFrame = useCallback(async () => {
    if (!videoRef.current || !detectionEnabled) return
    ...
    try {
      ...
      if (result.detections && result.detections.length > 0) {
        ...
        if ((result.has_violence || result.has_accident) && !alertShown) {
          ...
          const newIncident = {
            id: `cam-${Date.now()}`,
            type: result.has_violence ? 'violence' : 'accident',
  ...
  }, [detectionEnabled, drawDetections, alertShown, setIncidents, refreshAlerts])
    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-2">
        <input
          ...
          className="flex-1 min-w-0 px-3 py-2 rounded bg-gray-800 border border-gray-700 text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-green-500"
        />
        <button
          type="button"
          onClick={handleLoad}
        ...
        {feedUrl && (
          <button
            type="button"
            onClick={() => { setFeedUrl(null); setUrl(''); setError(null) }}
      ...
      <div className="rounded-lg overflow-hidden bg-black aspect-video relative">
        {isMjpg ? (
          <>
            ...
            <div className="absolute right-2 top-2 text-[10px] px-2 py-0.5 rounded" style={{ backgroundColor: 'rgba(220, 38, 38, 0.35)', color: '#fff' }}>
              LIVE
            </div>
            <div className="pointer-events-none absolute inset-0" style={{
              backgroundImage: 'repeating-linear-gradient(0deg, rgba(255,255,255,0.03) 0, rgba(255,255,255,0.03) 1px, transparent 1px, transparent 2px), repeating-linear-gradient(90deg, rgba(255,255,255,0.03) 0, rgba(255,255,255,0.03) 1px, transparent 1px, transparent 2px)',
